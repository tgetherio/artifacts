...
interface tgetherMembersContractInterface{
    function getMemberCreds(address _memberAddress, string memory _communityName) external view returns (uint256 creds); 
}

interface tgetherArtifactsInterface{
    function getArtifactExists(uint256 _artifactId) external view returns(bool);
}

interface laneRegistryContractInterface{
...
interface LaneInterface{
    function removeArtifact(uint256 proposalId) external;
    function getartifactIndex(uint256 artifactId) external view returns (uint256); 
}
contract tgetherArtifactConsensus{
    ...
    mapping (uint256=>uint256) public CommunitySubmissionToArtifact; // communitySubmissionId => artifactID
    uint256 public csCounter;

    // mapping allows us to retrieve the community submissions for a specifc artifact
    mapping( uint256  => uint256[] ) public ArtifactSubmissionList; // artifactId => communitySubmissionId

    // mapping allows us to retrieve the artifact submissions for a specific community
    mapping(uint256=> mapping(string => uint256)) public artifactCommunities; // artifactId => communityName => communitySubmissionId
    ...
    // mapping allows us to track if someone has already reviewed a artifact (avoid spam)
    ...
    tgetherMembersContractInterface public MembersContract; 
    tgetherArtifactsInterface public ArtifactsContract;
    ...
    constructor(address _CommunityConsensusAddress, address _MembersContract, address _ArtifactContract, uint256 _fee ) {
        owner= msg.sender;

        CommunityConsensusContract= tgetherCommunityConsensusInterface(_CommunityConsensusAddress);
        MembersContract= tgetherMembersContractInterface(_MembersContract);
        ArtifactsContract= tgetherArtifactsInterface(_ArtifactContract);
    ...
    // Events
    event ArtifactSubmission(uint256 indexed artifactId, uint256 indexed communitySubmissionId);
    ...
    event ConsensusUpdated(uint256 indexed communitySubmissionId, uint256 artifactId, string newConsensus);
    event Keywords(uint256 artifactId, string keyword);

    


    /*
    @notice Submits a artifact to a community for review
    @dev Checks if the artifact exists, if the artifact has already been submitted to the community, and if the community exists
    @param _artifactId uint256 artifact id to be submitted
    ...
    * execute your upkeep at any time by calling the manualUpkeepArtifact function.
    *
...
function submitToCommunity(uint256 _artifactId, string memory _communityName) external payable returns (uint256 )  {
    // Check if the artifact exists
    require(ArtifactsContract.getArtifactExists(_artifactId), "Artifact does not exist");
    ...
    if (consensusTime == 0 ){
        ...
        uint256 laneId = laneRegistryContractInterface(laneRegistry).appendToLane{value: msg.value}(csCounter);
    ...
    ArtifactSubmissionList[_artifactId].push(newCsCounter);
    CommunitySubmissionToArtifact[newCsCounter] = _artifactId;
    artifactCommunities[_artifactId][_communityName] = newCsCounter;

    // Increment csCounter only once after all operations
    csCounter++;

    // Emit events using newCsCounter
    emit ArtifactSubmission(_artifactId, newCsCounter);
    ...
    /*
    * @notice Adds a review to the reviews array for a specific artifact
    * @notice Consous value 2 will accept a artifact anything else will reject it
    ...
    * @param _artifactId uint256 artifact id review is attached to 
    ...
    */
    ...
    function submitReview(uint256 _communitySubmissionId, string memory _content, uint256 _consensus) public returns(uint256){
        ...
        // Avoid Spam
        require(!hasReviewed[_communitySubmissionId][msg.sender], "You've already reviewed this artifact");
        ...
        // Add the review to the artifact's reviews
    ...
    function _performSubmissionUpkeep(int256 resultId) internal {
        ...
        if (
            ...
            emit ConsensusUpdated(submissionId, CommunitySubmissionToArtifact[submissionId], CommunitySubmissions[submissionId].consensus == Consensus.Accepted ? "Accepted" : "Rejected");
    ...
    function performUpkeep(bytes calldata performData) external {
        ...
        address lane = laneRegistryContractInterface(laneRegistry).getLaneContractAddress(SubmissionLane[_submissionId]);
        require(msg.sender == lane, "Only lane contract can perform upkeep");
        require(LaneInterface(lane).getartifactIndex(_submissionId) >= 0, "Submission does not exist in lane");
     ...
     */
    function manualUpkeepArtifact(uint256 submissionId) external {
        ...
        address lane = laneRegistryContractInterface(laneRegistry).getLaneContractAddress(SubmissionLane[_submissionId]);
        LaneInterface(lane).removeArtifact(_submissionId);
    }
    ...
    function getArtifactSubmissionList(uint256 _artifactId) public view returns (uint256[] memory) {
        return ArtifactSubmissionList[_artifactId];
    }
    ...
    function getArtifactSubmissionCommunity(uint256 _artifactId) external view returns(string memory){
        return CommunitySubmissions[_artifactId].communityName;
    }
    function getArtifactSubmissionConsensus(uint256 _artifactId) external view returns(string memory){
        return CommunitySubmissions[_artifactId].communityName;
    }
    ...
    function setMembersContract(address _contractAddress) external ownerOnly {
        MembersContract= tgetherMembersContractInterface(_contractAddress);
    ...
    function setArtifactsContract(address _contractAddress) external ownerOnly {
        ArtifactsContract= tgetherArtifactsInterface(_contractAddress);
